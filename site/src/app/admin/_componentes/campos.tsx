...
import { PALETA, type Cor } from '@/lib/content/cores'
...
/**
 * Paleta fechada por padrão. Hex livre fica atrás de um clique a mais —
 * é o que mantém a identidade de pé entre as gestões.
 */
export function EscolhaCor({ rotulo, valor, onChange, permitirNenhuma }: { rotulo: string; valor: Cor | undefined; onChange: (v: Cor | undefined) => void; permitirNenhuma?: boolean }) {
  const nomes = Object.keys(PALETA) as (keyof typeof PALETA)[]
  const ehDaPaleta = valor === undefined || nomes.includes(valor as keyof typeof PALETA)
